...
const og = () => ({
  ...
  hooks: {
    "astro:build:done": async ({ dir, pages }) => {
      try {
        const brockmann = fs.readFileSync(
          "public/fonts/brockmann-semibold.woff",
        );
        ...
        for (const {pathname} of pages) {
          ...
          const file = fs.readFileSync(
            `src/content/projects/${pathname.slice(9, -1)}.mdx`
          );
        ...
        for (const {pathname} of pages) {
          ...
          const file = fs.readFileSync(
            `src/content/posts/${pathname.slice(6, -1)}.mdx`
          );
